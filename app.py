...
app.config["SQLALCHEMY_DATABASE_URI"] = "sqlite:///" + os.path.join(
    basedir, "grimaimDash_Finance.db"
)
...
# Routes Static Service Worker.JS
...
def dashboardBulanan():
    ...
    # Hitung Total Pengeluaran dan Pemasukan Bulan ini
    today = date.today()
    (
        total_pengeluaran_bulan_ini,
        total_pemasukan_bulan_ini,
    ) = get_total_pengeluaran_pemasukan_bulanan(user_id, today.year, today.month)

    # Hitung Total Pengeluaran dan Pemasukan Bulan Lalu
    last_month = today.replace(day=1) - timedelta(days=1)
    (
        total_pengeluaran_bulan_lalu,
        total_pemasukan_bulan_lalu,
    ) = get_total_pengeluaran_pemasukan_bulanan(
        user_id, last_month.year, last_month.month
    )
...
def tambahPengeluaran():
    ...
    if request.method == "POST":
        ...
        # Jika permintaan adalah GET, ambil info pengguna, kategori pengeluaran
        userInfo = get_userInfo(current_user.username)
...
def tambahPemasukan():
    ...
    if request.method == "POST":
        ...
        # Jika permintaan adalah GET, ambil info pengguna, kategori pemasukan
        userInfo = get_userInfo(current_user.username)
        kategori_list = Kategori.query.all()
...
def deletedAccount(username):
    ...
    # Menghapus semua data pengeluaran berdasarkan ID pengguna
    pengeluaran = Pengeluaran.query.filter_by(userID=current_user.userID).all()
    for pengeluaran in pengeluaran:
        db.session.delete(pengeluaran)

    # Menghapus semua data pemasukan berdasarkan ID pengguna
    pemasukan = Pemasukan.query.filter_by(userID=current_user.userID).all()
    for pemasukan in pemasukan:
        db.session.delete(pemasukan)
...
if __name__ == "__main__":
    app.run(debug=True, host="127.0.196.0", port=5000)

# flask run -h 127.0.196.0 -p 5000
# -------------------------------------------------------------------------------------------------------------------
